...
for $name in $node//country/name[matches(., $query, 'i')]
let $label := $name/text()
...
let $query := xdmp:get-request-field("query")
for $name in $node//region[@name[matches(., $query, 'i')]]
let $item := $name/@name/string()
return <result><label>{$item}</label><value>{$item}</value></result>
}
</results>